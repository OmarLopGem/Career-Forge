...
const { aiChat, aiChatJSON, AIServiceError, aiAnalyzeFile, hasProviderConfigured, createAIProvider, getProvider, resetProvider, getTokenUsage, resetTokenUsage } = await import('./ai.js')
...
describe('AI Service', () => {
  ...
  describe('chat interface', () => {
    ...
    it('parses JSON after a MiniMax thinking block', async () => {
      const provider = await createAIProvider()
      vi.spyOn(provider, 'chat').mockResolvedValue({
        content: '<think>Reasoning with {"draft":false} must not be parsed.</think>\n{"ok":true}',
        tokenUsage: { inputTokens: 8, outputTokens: 6, totalTokens: 14 },
        raw: {},
      })

      await expect(
        aiChatJSON({ system: 'Return JSON only.', user: 'Return an object.' }),
...
describe('aiAnalyzeFile (multimodal)', () => {
  beforeEach(() => {
    resetProvider()
    resetTokenUsage()
    vi.restoreAllMocks()
  ...
  it('parses JSON content returned by the provider', async () => {
    const provider = await createAIProvider()
    vi.spyOn(provider, 'analyzeFile').mockResolvedValue({
    ...
    expect(result.data).toBe('{"ok":true,"value":1}')
    expect(result.tokenUsage).toEqual({ inputTokens: 5, outputTokens: 5, totalTokens: 10 })
  })

  it('maps MISSING_API_KEY to AIServiceError', async () => {
    const provider = await createAIProvider()
    const err = new Error('missing')
    err.code = 'MISSING_API_KEY'
    vi.spyOn(provider, 'analyzeFile').mockRejectedValue(err)
    await expect(
      aiAnalyzeFile({
        buffer: new Uint8Array([1]),
        mimeType: 'application/pdf',
  ...
  it('maps 4xx to CLIENT_ERROR', async () => {
    const provider = await createAIProvider()
    ...
    ).rejects.toMatchObject({ code: 'CLIENT_ERROR', status: 400 })
  })
})

describe('hasProviderConfigured', () => {
  it('returns false for sample keys', () => {
    process.env.MINIMAX_API_KEY = 'sample'
    expect(hasProviderConfigured()).toBe(false)
  })
  ...
  it('returns true for a real key', () => {
    process.env.MINIMAX_API_KEY = 'sk-cp-real'
    expect(hasProviderConfigured()).toBe(true)
