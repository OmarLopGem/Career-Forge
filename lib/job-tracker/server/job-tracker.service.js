...
import {
  countActiveJobListings,
  getJobListingById,
  listActiveJobListings,
  listJobListings,
  upsertJobListings,
} from './job-listing.repository.js'
import {
  isAdzunaConfigured,
  sanitizeAdzunaSearchInput,
  searchAdzunaJobListings,
} from '@/lib/server/integrations/adzuna.js'
import {
  createCalendarEvent,
  deleteCalendarEvent,
  deleteCalendarEventsByJobApplicationId,
  getCalendarEventById,
  listCalendarEventsByUser,
  updateCalendarEvent,
} from './calendar-event.repository.js'

// The job tracker service is the orchestration layer for listings, applications,
// and calendar events. Routes stay thin while repositories stay persistence-only.
...
function profileToSnapshot(profile) {
  return {
    ...
    headline: String(profile.personalInfo?.headline ?? '').trim(),
    targetRole: String(profile.target?.desiredRole ?? '').trim(),
...
async function resolveCurrentUser() {
  // Auto-archive is triggered from normal reads/writes instead of a background job.
  const user = await requireCurrentUser()
  await autoArchiveForUser(user._id)
  return user
}

function buildListingSourceMeta(overrides) {
  return {
...
function buildListingPagination({ page, pageSize, total }) {
  const normalizedTotal = Math.max(0, Number.parseInt(total, 10) || 0)
  return {
    page,
    pageSize,
    total: normalizedTotal,
    totalPages: normalizedTotal === 0 ? 0 : Math.ceil(normalizedTotal / pageSize),
  }
}
...
async function syncApplicationStatusFromEvent(userId, eventInput, existingApplicationId = null) {
  ...
  if (!application.isArchived) {
    // Events push the application state machine forward, but deleting an event
    // later does not attempt to reverse that history.
...
export async function serviceListJobListings(input = {}) {
  await requireCurrentUser()
  const search = sanitizeAdzunaSearchInput(input)

  if (isAdzunaConfigured()) {
    try {
      const liveResult = await searchAdzunaJobListings(search)
      const syncedJobListings = await upsertJobListings(liveResult.jobListings)

      return {
        jobListings: syncedJobListings,
        search: liveResult.search,
        sourceMeta: liveResult.sourceMeta,
        pagination: buildListingPagination({
          page: liveResult.search.page,
          pageSize: liveResult.search.resultsPerPage,
          total: liveResult.sourceMeta.totalResults,
        }),
      }
    } catch (err) {
      const [fallbackJobListings, fallbackTotal] = await Promise.all([
        listActiveJobListings(search),
        countActiveJobListings(search),
      ])

      return {
        jobListings: fallbackJobListings,
        search,
        sourceMeta: buildListingSourceMeta({
          mode: 'fallback',
          fallbackUsed: true,
          reason: err instanceof Error ? err.message : 'Live Adzuna sync failed.',
        }),
        pagination: buildListingPagination({
          page: search.page,
  ...
  return {
    ...
    search,
    sourceMeta: buildListingSourceMeta({
      reason: 'Adzuna is not configured, so cached Mongo listings are being used.',
    }),
    pagination: buildListingPagination({
      page: search.page,
      pageSize: search.resultsPerPage,
...
export async function serviceListAdminJobListings() {
  await requireAdminUser()
  const jobListings = await listJobListings()

  return {
    jobListings,
    summary: {
      total: jobListings.length,
      active: jobListings.filter((listing) => listing.isActive).length,
      inactive: jobListings.filter((listing) => !listing.isActive).length,
    },
  }
}
...
export async function serviceCreateJobApplication(input) {
  ...
  const cvProfileId = String(input.cvProfileId ?? '').trim()
  ...
  if (!cvProfileId) {
    throw new AppServiceError(
      'cvProfileId is required when creating a job application.',
      'CV_PROFILE_REQUIRED',
      400,
    )
  }

  const cvProfile = ensureFound(
    await getProfileById(user._id, cvProfileId),
    'CV profile not found.',
    'CV_PROFILE_NOT_FOUND',
  )

  if (input.jobListingId) {
    // Applications created from the shared catalog still keep a snapshot so they
    // remain stable even if the original listing changes later.
  ...
  const application = await createJobApplication({
    ...
    cvProfileSnapshot: profileToSnapshot(cvProfile),
...
export async function serviceDeleteJobApplication(applicationId) {
  ...
  await deleteCalendarEventsByJobApplicationId(user._id, applicationId)
...
export async function serviceCreateCalendarEvent(input) {
  ...
  const event = await createCalendarEvent({
    userId: user._id,
    ...eventInput,
    // Give the UI a useful title when the user leaves the field blank.
