...
async function getCollection() {
  ...
  if (!indexes.some((index) => index.name === 'sessions_token_unique')) {
    await collection.createIndex({ token: 1 }, { unique: true, name: 'sessions_token_unique' })
  }

  const expiryIndex = indexes.find((index) => index.key?.expiresAt === 1)
  if (expiryIndex?.expireAfterSeconds !== 0) {
    if (expiryIndex?.name) {
