...
describe('users.repository.updateUserAccount', () => {
  it('updates account-level identity fields on the user document', async () => {
    const passwordHash = await hashPassword('password123')
    const user = await createUser({
      firstName: 'Ada',
      lastName: 'Lovelace',
    ...
    const updated = await updateUserAccount(user._id, {
      ...
      linkedinUrl: 'https://linkedin.com/in/adriana',
      githubUrl: 'https://github.com/adriana',
      portfolioUrl: 'https://adriana.dev',
    })

    expect(updated).not.toBeNull()
    expect(updated.firstName).toBe('Adriana')
    expect(updated.dateOfBirth).toBe('1994-10-21')
    expect(updated.photoUrl).toBe('https://example.com/avatar.png')
    expect(updated.linkedinUrl).toBe('https://linkedin.com/in/adriana')
  })
})
