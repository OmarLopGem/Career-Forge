...
async function getAuthenticatedUserFromRequest() {
  ...
  // Tests can bypass cookie handling by injecting a mock user id.
...
export async function requireAdminUser() {
  ...
  // Role checks stay server-side even if the client hides admin navigation.
  if (user.role !== 'admin') {
    throw new AppServiceError('Admin access required.', 'FORBIDDEN', 403)
  }
...
export async function requireEmployerUser() {
  ...
  // Role checks stay server-side even if the client hides employer navigation.
  if (user.role !== 'employer') {
    throw new AppServiceError('Employer access required.', 'FORBIDDEN', 403)
  }
