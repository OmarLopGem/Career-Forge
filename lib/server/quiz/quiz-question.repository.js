...
export async function listAdminQuizQuestionsPage({ page, pageSize }) {
  const Model = await getModel()
  ...
  const effectiveDifficulty = {
    $switch: {
      branches: [
        {
          case: { $in: ['$difficulty', QUIZ_DIFFICULTIES] },
          then: '$difficulty',
        },
        { case: { $eq: ['$type', 'short'] }, then: 'Advanced' },
        { case: { $eq: ['$type', 'blank'] }, then: 'Intermediate' },
      ],
      default: 'Beginner',
    },
  }

  const [docs, difficultyCounts] = await Promise.all([
    Model.find({})
      .sort({ createdAt: -1, _id: -1 })
      .skip(skip)
      .limit(pageSize),
    Model.aggregate([
      { $group: { _id: effectiveDifficulty, count: { $sum: 1 } } },
    ]),
  ])
  const counts = Object.fromEntries(
    difficultyCounts.map(({ _id, count }) => [_id, count]),
  )

  return {
    questions: docs.map(toQuizQuestion),
    summary: {
      total: difficultyCounts.reduce((total, entry) => total + entry.count, 0),
