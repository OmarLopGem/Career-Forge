...
describe('quiz.service', () => {
  ...
  it('paginates the admin question bank while returning full summary counts', async () => {
    const admin = await createUser({
      firstName: 'Admin',
      lastName: 'User',
    ...
    await seedQuizQuestions([
      ...Array.from({ length: 12 }, (_, index) => ({
        ...
        type: 'mcq',
        difficulty: 'Beginner',
        question: `Beginner admin question ${index + 1}`,
      ...
      ...Array.from({ length: 8 }, (_, index) => ({
        ...
        type: 'blank',
        difficulty: 'Intermediate',
        question: `Intermediate admin question ${index + 1}`,
        answer: 'Correct',
        marks: 1,
      })),
      ...Array.from({ length: 5 }, (_, index) => ({
        ...
        type: 'short',
        difficulty: 'Advanced',
        question: `Advanced admin question ${index + 1}`,
        answer: 'Correct',
        marks: 1,
      })),
    ])

    const listing = await serviceListAdminQuizQuestions({ page: 2, pageSize: 10 })

    expect(listing.questions).toHaveLength(10)
