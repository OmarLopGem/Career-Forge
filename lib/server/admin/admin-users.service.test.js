...
describe('admin-users.service', () => {
  ...
  it('admin can warn a user and the user receives the notice in Profile Hub data', async () => {
    const admin = await createTestUser({
      firstName: 'Admin',
      email: 'admin@example.com',
      role: 'admin',
  ...
  it('admin can revoke a deleted user\'s access while retaining an audit record', async () => {
    ...
    const { getDb } = await import('@/lib/cv-assistant/server/mongo.js')
    const db = await getDb()
    expect(await db.collection('sessions').countDocuments({ userId: target._id })).toBe(0)
  })
