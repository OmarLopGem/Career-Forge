...
import { createQuizResult, getBestPercentageByJobType, listQuizResultsByUser } from './quiz-result.repository.js'
...
function extractAnalysisScore(analysis) {
  const atsScore = Number(analysis?.atsFeedback?.score)
  if (Number.isFinite(atsScore)) return roundMetric(atsScore)

  const overallScore = Number(analysis?.overallScore)
  if (Number.isFinite(overallScore)) return roundMetric(overallScore)
...
export async function serviceGetProgressOverview() {
  const user = await requireCurrentUser()
  const [applications, quizResults, profiles, bestByJobType] = await Promise.all([
    listJobApplicationsByUser(user._id),
    listQuizResultsByUser(user._id),
    listProfilesByUser(user._id),
    getBestPercentageByJobType(user._id),
  ])

  const profileProgress = await Promise.all(
    profiles.map(async (profile) => {
      const analyses = await listAnalysesByProfile(user._id, profile._id)
      const scoreHistory = analyses
        .map((analysis) => ({
          _id: analysis._id,
          createdAt: analysis.createdAt,
          score: extractAnalysisScore(analysis),
        }))
        .filter((entry) => entry.score !== null)
        .sort(sortByCreatedAtAsc)
      const firstScoreEntry = scoreHistory[0] ?? null
      const latestScoreEntry = scoreHistory[scoreHistory.length - 1] ?? null
      const previousScoreEntry = scoreHistory.length > 1
        ? scoreHistory[scoreHistory.length - 2]
        : null
      const bestAnalysisScore = scoreHistory.length > 0
        ? Math.max(...scoreHistory.map((entry) => entry.score))
        : null
      ...
      return {
        ...
        lastAnalysisScore: latestScoreEntry?.score ?? null,
        bestAnalysisScore,
        analysisCount: scoreHistory.length,
        latestChange: previousScoreEntry
          ? roundMetric(latestScoreEntry.score - previousScoreEntry.score)
          : null,
        improvementSinceFirst: firstScoreEntry && latestScoreEntry
          ? roundMetric(latestScoreEntry.score - firstScoreEntry.score)
          : null,
        scoreHistory,
