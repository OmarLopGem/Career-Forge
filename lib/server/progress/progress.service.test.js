...
function sampleAnalysisDraft(score) {
  return {
    atsFeedback: {
      score,
      comments: [`ATS score ${score}`],
      formattingWarnings: [],
...
describe('progress.service', () => {
  ...
  it('builds a private progress overview from quiz, profile, and job data', async () => {
    ...
    await createAnalysisFromDraft(user._id, profile._id, sampleAnalysisDraft(68))
    await new Promise((resolve) => setTimeout(resolve, 5))
    await createAnalysisFromDraft(user._id, profile._id, sampleAnalysisDraft(84))
    ...
    expect(overview.profileProgress[0]).toMatchObject({
      title: 'Frontend Profile',
    ...
    expect(overview.profileProgress[0].scoreHistory).toHaveLength(2)
    expect(overview.profileProgress[0].scoreHistory[0].score).toBe(68)
    expect(overview.profileProgress[0].scoreHistory[1].score).toBe(84)
  })
  ...
  it('only includes the current user CV score history in the progress overview', async () => {
    const user = await createScopedUser('history-owner@example.com')
    const profile = await createProfile({
      ...frontendProfile,
      userId: user._id,
      title: 'Owner Profile',
    })
    await createAnalysisFromDraft(user._id, profile._id, sampleAnalysisDraft(73))
    ...
    await createAnalysisFromDraft(otherUser._id, otherProfile._id, sampleAnalysisDraft(91))

    process.env.MOCK_USER_ID = user._id
    const overview = await serviceGetProgressOverview()

    expect(overview.profileProgress).toHaveLength(1)
    expect(overview.profileProgress[0].title).toBe('Owner Profile')
    expect(overview.profileProgress[0].scoreHistory).toHaveLength(1)
    expect(overview.profileProgress[0].scoreHistory[0].score).toBe(73)
  })
})
