...
export async function createQuizResult(data) {
  ...
  const docData = {
    ...
    passed: data.passed,
    gradingMode: data.gradingMode ?? 'answer-key',
...
export async function listQuizResultsByUserAndJobType(userId, jobType) {
  const Model = await getModel()
  const docs = await Model.find({ userId, jobType }).sort({ completedAt: -1, createdAt: -1 })
  return docs.map(toQuizResult)
}
