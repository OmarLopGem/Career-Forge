...
async function countDocuments(getModel, filter = {}) {
...
describe('support.service', () => {
  it('lets a user create a ticket with the first message persisted', async () => {
    ...
    const ticketsCount = await countDocuments(getSupportTicketModel)
    const messagesCount = await countDocuments(getSupportMessageModel, {
  ...
  it('rejects creating a 6th active ticket', async () => {
    ...
    const total = await countDocuments(getSupportTicketModel)
    expect(total).toBe(5)
  })
