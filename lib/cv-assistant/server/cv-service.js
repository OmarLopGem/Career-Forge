...
export async function serviceImportCV(input, dependencies = {}) {
  const userId = await getCurrentUserId()
  try {
    const result = await importCVFromBuffer(input, userId, dependencies)
...
export async function serviceDeleteProfile(profileId) {
  ...
  await deleteAnalysesByProfile(userId, profileId)
  return { ok: true }
}
