...
vi.mock('./ai/parse-cv-file-to-profile.js', () => ({
  parseCVFileToProfile: async () => ({
    personalInfo: {
      fullName: 'Jane Doe',
    ...
    experience: [
      {
        ...
        highlights: [
          ...
          'Led the migration of the dashboard to React Server Components.',
        ],
    ...
    projects: [
      {
        name: 'Open UI Kit',
        description: 'Component library for the open source community.',
    ...
    links: [
      ...
      { label: 'LinkedIn', url: 'https://linkedin.com/in/jane', type: 'linkedin' },
    ],
  }),
}))
...
const {
  ...
  toApiErrorResponse,
} = await import('./cv-service.js')
import { startMongo, stopMongo, clearMongo } from '../test/mongo-helpers.js'
import { getLatestAnalysis as getLatestAnalysisFromRepository } from './cv-analysis.repository.js'
...
describe('cv-service', () => {
  ...
  it('importCV rejects empty or too-short profile names', async () => {
    try {
      await serviceImportCV({
        fileName: 'cv.pdf',
        mimeType: 'application/pdf',
  ...
  it('importCV rejects missing profile names', async () => {
    try {
      await serviceImportCV({
        fileName: 'cv.pdf',
        mimeType: 'application/pdf',
  ...
  it('listProfiles is scoped to mock user', async () => {
    await serviceImportCV({ fileName: 'cv.pdf', mimeType: 'application/pdf', buffer: makeBuffer(), title: 'Test Profile' })
  ...
  it('getProfile validates ownership and throws 404 for another user id (mocked)', async () => {
    const created = await serviceImportCV({
      fileName: 'cv.pdf', mimeType: 'application/pdf', buffer: makeBuffer(), title: 'Test Profile',
    })
  ...
  it('updateProfile stores a profile-specific professional summary', async () => {
    const created = await serviceImportCV({
      fileName: 'cv.pdf', mimeType: 'application/pdf', buffer: makeBuffer(), title: 'Test Profile',
    })

    const updated = await serviceUpdateProfile(created.profile._id, {
      professionalSummary: 'Backend-focused profile for platform engineering roles.',
    })

    expect(updated.profile.professionalSummary).toBe(
      'Backend-focused profile for platform engineering roles.',
    )
  })
  ...
  it('deleteProfile removes profile', async () => {
    ...
    const analysis = await getLatestAnalysisFromRepository('mock-user-cv-assistant', created.profile._id)
    expect(analysis).toBeNull()
  })

  it('deleteProfile also removes saved analyses for that profile', async () => {
    const created = await serviceImportCV({
      fileName: 'cv.pdf', mimeType: 'application/pdf', buffer: makeBuffer(), title: 'Test Profile',
    })
    await serviceAnalyzeProfile(created.profile._id)
