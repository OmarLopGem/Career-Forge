...
describe('cv-profile repository', () => {
  ...
  it('preserves existing profile data when patching unrelated fields', async () => {
    const p = await createProfile({ ...frontendProfile, userId: 'user-A' })
    const updated = await updateProfile('user-A', p._id, { title: 'Retitled profile' })

    expect(updated?.title).toBe('Retitled profile')
    expect(updated?.personalInfo.fullName).toBe(frontendProfile.personalInfo.fullName)
    expect(updated?.completion.isMinimumComplete).toBe(true)
  })
  ...
  it('reassigns the default profile when the current default is deleted', async () => {
    const first = await createProfile({ ...frontendProfile, userId: 'user-A', isDefault: true })
    const second = await createProfile({ ...minimalValidProfile, userId: 'user-A', isDefault: false })

    expect(await deleteProfile('user-A', first._id)).toBe(true)
  ...
  it('automatically keeps only one default profile when creating multiple profiles', async () => {
    const first = await createProfile({ ...frontendProfile, userId: 'user-A', isDefault: true })
    const second = await createProfile({ ...minimalValidProfile, userId: 'user-A', isDefault: true })
    const list = await listProfilesByUser('user-A')
    const updatedFirst = list.find((profile) => profile._id === first._id)
    const updatedSecond = list.find((profile) => profile._id === second._id)
