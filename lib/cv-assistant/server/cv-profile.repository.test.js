...
describe('cv-profile repository', () => {
  ...
  it('keeps completion data consistent when updating unrelated fields', async () => {
    const p = await createProfile({ ...frontendProfile, userId: 'user-A' })
    const updated = await updateProfile('user-A', p._id, { title: 'Updated title' })

    expect(updated?.title).toBe('Updated title')
    expect(updated?.completion.isMinimumComplete).toBe(true)
    expect(updated?.completion.score).toBeGreaterThan(0)
  })
  ...
  it('keeps only one default profile when creating multiple defaults', async () => {
