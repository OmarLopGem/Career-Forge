/**
 * Real CV profile analysis powered by the configured AI provider
 * (defaults to MiniMax via the OpenAI-compatible SDK).
 *
 * The output contract is the same CVAnalysisDraft that the previous mock
 * implementation produced, so downstream persistence (cv_analyses) and
 * client code keep working unchanged.
 */
...
export class CVAnalysisError extends Error {
  constructor(message, code = 'ANALYSIS_FAILED', details) {
    super(message)
...
export async function analyzeCVProfile(input) {
  const { profile } = input
  const fallback = buildFallbackDraft(profile)

  try {
    const ai = await generateAnalysisFromAI(profile)
    return mergeDraft(fallback, ai, profile)
  } catch (err) {
    if (err instanceof AIServiceError) {
      console.warn('[analyzeCVProfile] AI provider failed, using fallback:', err.message)
      return fallback
    }
    throw err
  }
...
async function generateAnalysisFromAI(profile) {
  const system = [
    'You are a senior technical recruiter and CV analyst.',
    'You analyze CV profiles and return strict JSON only.',
    'You never invent skills, jobs, or achievements that are not present in the profile.',
  ].join(' ')

  const user = [
    'Analyze the following CV profile and return JSON with exactly this shape:',
    '{',
    '  "detectedNiche": string,',
    '  "nicheReasoning": string,',
    '  "strengths": [{ "title": string, "description": string, "section": string, "priority": "low"|"medium"|"high" }],',
    '  "weaknesses": [{ "title": string, "description": string, "section": string, "priority": "low"|"medium"|"high" }],',
    '  "improvementSuggestions": [{ "title": string, "description": string, "section": string, "priority": "low"|"medium"|"high" }],',
    '  "atsFeedback": { "score": number 0-100, "comments": string[], "formattingWarnings": string[], "keywordSuggestions": string[] },',
    '  "suggestedRoles": [{ "title": string, "fitScore": number 0-1, "reasoning": string }],',
    '  "keywordGaps": string[]',
    '}',
    '',
    'Be specific. Use the profile data verbatim. Return only the JSON object.',
    '',
    'Profile:',
    JSON.stringify(profile, null, 2),
  ].join('\n')

  const { data } = await aiChatJSON({ system, user, temperature: 0.2 })
  return data
}

function mergeDraft(fallback, ai, profile) {
  return {
    detectedNiche: ai.detectedNiche || fallback.detectedNiche,
    nicheConfidence: 0.85,
    nicheReasoning: ai.nicheReasoning || fallback.nicheReasoning,
    strengths: sanitizeItems(ai.strengths) ?? fallback.strengths,
    weaknesses: sanitizeItems(ai.weaknesses) ?? fallback.weaknesses,
    improvementSuggestions:
      sanitizeItems(ai.improvementSuggestions) ?? fallback.improvementSuggestions,
    atsFeedback: sanitizeAts(ai.atsFeedback) ?? fallback.atsFeedback,
    suggestedRoles: sanitizeRoles(ai.suggestedRoles) ?? fallback.suggestedRoles,
    keywordGaps:
      Array.isArray(ai.keywordGaps) && ai.keywordGaps.length > 0
        ? ai.keywordGaps
        : fallback.keywordGaps,
  }
}
...
function sanitizeAts(ats) {
  if (!ats || typeof ats !== 'object') return undefined
  const score =
    typeof ats.score === 'number'
      ? Math.max(0, Math.min(100, Math.round(ats.score)))
      : undefined
  if (score === undefined) return undefined
  return {
    score,
    comments: Array.isArray(ats.comments) ? ats.comments : [],
    formattingWarnings: Array.isArray(ats.formattingWarnings)
      ? ats.formattingWarnings
      : [],
    keywordSuggestions: Array.isArray(ats.keywordSuggestions)
      ? ats.keywordSuggestions
      : [],
  }
}

function buildFallbackDraft(profile) {
  return {
