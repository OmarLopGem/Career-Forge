...
async function generateDraftFromAI(text) {
  const system = [
    ...
    'You never invent facts. If a field is missing, return an empty string or empty array.',
  ...
  const { data } = await aiChatJSON({ system, user, temperature: 0.1 })
  return data
}
...
function mergeDraft(fallback, ai, isTechnical, hasLeadership) {
  ...
  return {
    ...
    experience: sanitizeExperience(ai.experience) ?? fallback.experience,
    education: sanitizeEducation(ai.education) ?? fallback.education,
    skills: sanitizeSkills(ai.skills) ?? fallback.skills,
...
function sanitizeCertifications(certs) {
