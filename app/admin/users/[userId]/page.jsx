...
=======
        <section className="rounded-[2rem] border border-border bg-surface p-5 shadow-sm sm:p-8">
          <div className="flex flex-col gap-2">
            ...
            <h2 className="text-2xl font-bold text-navy">CV workspaces connected to this user</h2>
          </div>
              ...
              {profiles.map((profile) => (
                <article key={profile._id} className="rounded-3xl border border-border bg-background p-5">
                  ...
                  <div className="mt-5 flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-navy">{profile.completionScore}%</span>
                  </div>

                  <p className="mt-4 text-sm text-text-muted">Updated {formatDate(profile.updatedAt)}</p>
                </article>
              ))}
            </div>
          )}
